...
with open("usuarios.txt", "r") as arquivo:
    ...
    usuarios = [[usuario[0], int(usuario[1])] for usuario in usuarios]
    usuarios = sorted(usuarios, key=lambda usuario: usuario[1], reverse=True)
    total = sum([usuario[1] for usuario in usuarios])
    ...
    with open("relatorio.txt", "w") as relatorio:
        ...
        for i, usuario in enumerate(usuarios):
            relatorio.write(f"{i + 1:<4} {usuario[0]:<15} {usuario[1] / 1024 / 1024:10.2f} MB {usuario[1] / total * 100:10.2f}%\n")
